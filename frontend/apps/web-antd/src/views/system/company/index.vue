...
import { getSystemCompanyList, deleteSystemCompany, type SystemCompanyApi } from '#/api/system/company';
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          const res = await getSystemCompanyList({
...
function onEdit(row: SystemCompanyApi.Company) {
  formDrawerApi.setData(row).open();
}

async function onDelete(row: SystemCompanyApi.Company) {
  try {
