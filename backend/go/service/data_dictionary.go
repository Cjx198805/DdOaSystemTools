...
func (s *DataDictionaryService) Create(dataDictionary *model.DataDictionary) error {
	...
	// 设置默认值
	if dataDictionary.Status == 0 {
		dataDictionary.Status = 1
	}
	if dataDictionary.DataType == "" {
		dataDictionary.DataType = "string"
	}
	if dataDictionary.Required == 0 {
		dataDictionary.Required = 0
	}
	if dataDictionary.Editable == 0 {
		dataDictionary.Editable = 1
	}
